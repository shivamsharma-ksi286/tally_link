...
frappe.ui.form.on("Tally Ledger", {
	refresh(frm) {
		...
		if (!frm.is_new()) {
			...
			frm.add_custom_button(__("View Sync Logs"), function () {
				frappe.route_options = {
					entity_name: frm.doc.ledger_name,
					entity_type: "Ledger"
				};
	...
	transmit_to_tally(frm) {
		frappe.confirm(
			__("Transmit ledger <b>{0}</b> to Tally?", [frm.doc.ledger_name]),
			function () {
				frappe.call({
					method: "tally_link.tally.doctype_sync.sync_ledger_to_tally",
					args: {
						ledger_name: frm.doc.name,
						operation: frappe.db.exists("Tally Ledger", frm.doc.name) ? "update" : "create"
					},
					freeze: true,
					...
					callback(r) {
						if (r.message && r.message.success) {
							frappe.show_alert({
								message: __("Ledger transmitted to Tally successfully."),
								indicator: "green"
							}, 5);
							frm.reload_doc();
						} else {
